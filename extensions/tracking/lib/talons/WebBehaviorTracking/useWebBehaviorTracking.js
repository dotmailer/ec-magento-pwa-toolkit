...
const useWebBehaviourTracking = () => {
    ...
    const { ...componentData } = talonProps;
    const { urlKey } = useStoreSlug();
    const { loading: productsLoading, data: productData } = useProduct(
        urlKey,
        componentData
    );
    const { loading: brandLoading, data: brandData } = useProductBrand(
        collection
    );
    ...
    useEffect(() => {
        if (productData) {
            productData.products.items.forEach(product =>
                dispatch({ type: 'ADD_PRODUCT', product })
            );
        }

        if (brandData) {
            brandData?.productBrands?.items?.forEach(brand =>
    ...
    useEffect(() => {
        ...
        if (data && data.trackingData.wbt_profile_id) {
            ...
            if (typeof window.dmPt == 'function') {
                let trackingData = {};
                if (!collection.empty) {
                    trackingData = collection
                        .getByTypeOrFirst('SimpleProduct')
                        .toTrackingData();
                }
